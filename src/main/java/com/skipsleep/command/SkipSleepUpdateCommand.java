...
public class SkipSleepUpdateCommand implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender,
        ...
        if (!(sender.hasPermission("SkipSleep.command.skipsleepupdate"))) {
        ...
        switch (args[0]) {
            case "on":
                SkipSleep.config.set("update", true);
                break;
            case "off":
                SkipSleep.config.set("update", false);
                break;
            default:
